...
*/
{
  ...
  if((index > n_remaining_items) || (upper_sum < sizes[remaining_items[n_remaining_items]]))
  {
    ...
    if(n_loads[bin] > MAX_N_LOADS)
    {
      #ifdef OUTPUT_SEARCH // JPG.20200518
      fprintf(stderr,"Maximum number of loads exceeded\n");
      #endif
      bin_status = 1;
      return;
    }
...
*/
{
  ...
  for(i = 1; i <= n_available_items; i++)
  {
    ...
    */
    ...
    if((index > 1) && (size_j < open_bins[index-1].remaining_size))
    {
      fprintf(stderr,"Incorrect index in best_fit_decreasing\n");
      exit(1);
    }

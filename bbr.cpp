...
int main(int ac, char **av)
{
  ...
  printf("lb1: %d lb2: %d lb3: %d bpp: %d hoffmann: %d UB: %d verified_optimality %d nodes: %d cpu: %6.2f\n",salb::search_info.lb_1,salb::search_info.lb_2,salb::search_info.lb_3,salb::search_info.lb_bin,salb::search_info.ub_hoffmann,
          salb::UB,salb::verified_optimality,salb::search_info.n_explored,cpu);  //JPG.Note: output linked with another exit found in memory.cpp

  current_time = time(NULL);
  if (salb::prn_info > 0) printf("\n%s end at %s\n", av[ac-1], ctime(&current_time));

  #ifdef OUTPUT_SEARCH
  salb::write_results((char*)"results.out",cpu);   //added by AS 2013/06/06
  #endif
...
void parseargs(int ac, char **av)
{
  ...
  cnt = 0;
  cycle=0; //JPG.20200430 change to overide cycle time form file (only when cycle=0 we use the cycle time in the instance.
  ...
  prob_file = av[cnt++];
  if (cnt < ac) usage (*av);
  printf("%s %d %d\t",prob_file,cycle,run_forward); //JPG.20210416 changed some output to report results
  fflush(stdout);
}
...
void testprob()
{
  int      count,  i, j, min_n_stations, n_stations, *stations, sum, t_sum, upper_bound; //JPG.20200430 iter and graph were not used, removed
  ...
  if (run_forward == -1)
  {
    ...
    //JPG.20200329 The original code overflows. The commented out code is the original code plus a test to show it (use an instance with n=1000 and low order strength
    //  the new version does not overflow for any of the n_1000 insntaces (but the tests are maintained for extra safety
    //  Note that the error was not critical, it only made the decision for forward or backward direction to be basically random
    /*
    ...
    */
    unsigned long f = 1;			//changed by AS 2013/06/10
    unsigned long r = 1;			//changed by AS 2013/06/10
    unsigned long fold=0;
    unsigned long rold=0;
    for (int m = 1; m <= 5; ++m)
    {
      unsigned int fcount = 0;      //changed by AS 2013/06/08
      unsigned int rcount = 0;		//changed by AS 2013/06/08
    ...
    if (r < f)
    {
      //printf("running in reverse %ld %ld\n", f, r); //JPG.20200429 removed output from screen
      reverse_pred();
      for (int j = 1; j <= n_tasks; ++j)
  ...
  {
    //printf("running in reverse\n");  //JPG.20200429 removed output from screen
    reverse_pred();
    for (int j = 1; j <= n_tasks; ++j)
      free(closed_predecessor_matrix[j]);
    free(closed_predecessor_matrix);
  ...
  search_info.ub_hoffmann=UB; // JPG.20200430 save partial results in search_info typedef
...
#endif
  //JPG.20200430 These calculations are made here to ensure that these bounds are calculated for each instance (due to reporting scheme)
  search_info.lb_1=(int)ceil((double)t_sum / (double)cycle);
  j=0;
  for(i=1;i<=n_tasks;i++)
  {
    if((t[i]*2)>cycle) j=j+2;
    if((t[i]*2)==cycle) j++;
  }
  search_info.lb_2=(int)ceil((double)(j) / 2.0);
  j=0;
  for(i=1;i<=n_tasks;i++)
  {
    if((t[i]*3)>(2*cycle)) j=j+6;
    if((t[i]*3)==(2*cycle)) j=j+4;
    if(((t[i]*3)>cycle)&&((t[i]*3)<(2*cycle))) j=j+3;
    if((t[i]*3)==cycle) j=j+2;
  }
  search_info.lb_3=(int)ceil((double)(j) / 6.0);
  //JPG. 20200430 End. Note: the method tries to reduce floating point arithmetic to a minimum

  global_start_time = clock();

  //JPG.20200518 change output
  search_info.lb_bin=search_info.lb_1;
  if(search_info.lb_bin<search_info.lb_2) search_info.lb_bin=search_info.lb_2;
  if(search_info.lb_bin<search_info.lb_3) search_info.lb_bin=search_info.lb_3;
  ...
  */
  ...
  #endif
  search_info.n_explored += n_explored; //JPG.20200429 I want to check how many states were explored between both calls (best_first and bfs)

  total_cpu = (double) (clock() - search_info.start_time) / CLOCKS_PER_SEC;

  /* JPG.20200430 removed
  ...
  */
  //write_solution(prob_file);		//added by AS 2013/06/06 (commented by JPG 20211106)
...
*/
{
  ...
  for(i = 1; i <= n_tasks; i++)
  {
    ...
    if((cycle / 3.0 < t[i]) && (t[i] < 2 * cycle / 3.0))
    {
      LB3_values[i] = 0.5;
      continue;
    }

...
{
  //int      i; //JPG.20200430 removed as it is unused
...
//void store_state(char *degrees, char n_stations, char LB, int idle, int hash_value, int previous) JPG.20200430 (char to int)
void store_state(char *degrees, int n_stations, int LB, int idle, int hash_value, int previous)
/*
...
*/
{
  ...
  if (last_state >= STATE_SPACE)
  {
    ...
    #endif
    printf("lb1: %d lb2: %d lb3: %d bpp: %d hoffmann: %d UB: %d verified_optimality %d nodes: %d cpu: %6.2f\n",salb::search_info.lb_1,salb::search_info.lb_2,salb::search_info.lb_3,salb::search_info.lb_bin,salb::search_info.ub_hoffmann,
          salb::UB,salb::verified_optimality,salb::search_info.n_explored,((double)(clock() - global_start_time)/CLOCKS_PER_SEC));  //JPG.Note: output linked with another exit found in memory.cpp
    #ifdef OUTPUT_SEARCH
    printf("   %d   verified_optimality = %d; value = %d; cpu = %0.2f out_of_memory\n",search_info.n_explored,verified_optimality, UB, ((double)(clock() - global_start_time)/CLOCKS_PER_SEC));
...
/* */
//_________________________________________________________________________________________________
//int find_or_insert(double key, char *degrees, char n_stations, char LB, int idle, int hash_value, int previous, int method, int *status) JPG.20200430 (char to int)
int find_or_insert(double key, char *degrees, int n_stations, int LB, int idle, int hash_value, int previous, int method, int *status)
/*
...
*/
{
  ...
  while ((index = hash_table[hash_index].index) != -1)
  {
    if (memcmp(states[index].degrees+1, degrees+1, n_tasks) == 0)
    {
      if(n_stations < states[index].n_stations)
      {
        ...
        assert(LB<states[index].LB); //JPG. 20200923 lower bound must be updated (otherwise error with n_50_480 and c=886)
        states[index].LB = LB; //JPG. 20200923 lower bound must be updated
        *status = 3;
        if((states[index].open == 0) && (method == 1))
        {
          ...
          {
          case 1:
            insert(&dbfs_heaps[n_stations], heap_sizes + n_stations, key, degrees, n_stations, LB, idle, hash_value, previous, 0);
            break;
          case 2:
            insert(&bfs_heap, heap_sizes, key, degrees, n_stations, LB, idle, hash_value, previous, 0);
            break;
          default:
            fprintf(stderr,"Unknown search_strategy in find_or_insert\n");
            exit(1);
            break;
          }
        }
  ...
  if(method == 1)
  {
    ...
    {
    case 1:
      insert(&dbfs_heaps[n_stations], heap_sizes + n_stations, key, degrees, n_stations, LB, idle, hash_value, previous, 1);
      break;
    case 2:
      insert(&bfs_heap, heap_sizes, key, degrees, n_stations, LB, idle, hash_value, previous, 1);
      break;
    default:
      fprintf(stderr,"Unknown search_strategy in find_or_insert\n");
      exit(1);
...
void initialize_heaps()
{
  ...
  {
  case 1:
    MALLOC(dbfs_heaps, n_tasks+1, heap_record *);
  ...
  default:
    fprintf(stderr,"Unknown search_strategy in initialize_heaps\n");
    exit(1);
    break;
  }
}
...
void reinitialize_heaps()
{
  ...
  {
  case 1:
    for(i = 0; i <= n_tasks; i++) dbfs_heaps[i][0].index = 0;
    break;
  case 2:
    bfs_heap[0].index = 0;
    break;
  default:
    fprintf(stderr,"Unknown search_strategy in reinitialize_heaps\n");
    exit(1);
    break;
  }
}
...
void free_heaps()
{
  ...
  {
  case 1:
    for(i = 0; i <= n_tasks; i++) free(dbfs_heaps[i]);
    free(dbfs_heaps);
    break;
  case 2:
    free(bfs_heap);
    break;
  default:
    fprintf(stderr,"Unknown search_strategy in free_heaps\n");
    exit(1);
    break;
  }
  free(heap_sizes);
...
*/
{
  ...
  case 2:
    ...
    break;
  default:
    fprintf(stderr,"Unknown search_strategy in get_min\n");
    exit(1);
    break;
  }
}
...
//_________________________________________________________________________________________________
//void insert(heap_record** heap, int* heap_size, double key, char *degrees, char n_stations, char LB, int idle, int hash_value, int previous, int add_to_states) JPG.20200430 (char to int)
void insert(heap_record** heap, int* heap_size, double key, char *degrees, int n_stations, int LB, int idle, int hash_value, int previous, int add_to_states)
/*
